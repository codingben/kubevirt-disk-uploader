...
func run(client kubecli.KubevirtClient, vmNamespace, vmName, volumeName, imageDestination string, pushTimeout int) error {
	...
	if err := disk.CreateVirtualMachineExportSecret(client, vmNamespace, vmName); err != nil {
		return err
	}

	log.Printf("Creating a new VirtualMachineExport '%s/%s' object...", vmNamespace, vmName)

	if err := disk.CreateVirtualMachineExport(client, vmNamespace, vmName); err != nil {
		return err
	}

	log.Println("Waiting for VirtualMachineExport status to be ready...")

	if err := disk.WaitUntilVirtualMachineExportReady(client, vmNamespace, vmName); err != nil {
		return err
	}

	log.Println("Getting raw disk URL from the VirtualMachineExport object status...")

	rawDiskUrl, err := disk.GetRawDiskUrlFromVolumes(client, vmNamespace, vmName, volumeName)
	if err != nil {
		return err
	}
	...
	certificateData, err := disk.GetCertificateFromVirtualMachineExport(client, vmNamespace, vmName)
	if err != nil {
		return err
	}

	if err := disk.CreateFile(certificatePath, certificateData); err != nil {
		return err
	}

	log.Println("Getting export token from the Secret object...")

	kvExportToken, err := disk.GetTokenFromVirtualMachineExportSecret(client, vmNamespace, vmName)
	if err != nil {
		return err
	}
